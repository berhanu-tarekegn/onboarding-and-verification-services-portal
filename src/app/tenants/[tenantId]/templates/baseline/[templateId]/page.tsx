...
export default async function BaselineTemplateDetailPage({
  params,
}: {
  params: Promise<{ tenantId: string; templateId: string }>;
}) {
  const { tenantId, templateId } = await params;
  const baseline = await api.getBaselineTemplate(templateId);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-xl border border-zinc-200 dark:border-slate-800 shadow-sm">
        ...
        <Link
          href={`/tenants/${tenantId}/templates/new?baselineId=${templateId}`}
          className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-500 transition-all flex items-center gap-2"
        >
